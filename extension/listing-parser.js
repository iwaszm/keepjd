(() => {
  ...
  function extractSearchPageObservations(html, capturedAt = captureDate()) {
    const found = new Map();
    ...
    for (const scriptText of scripts) {
      collectFromProductUrls(scriptText, found, capturedAt);
      collectFromStructuredIds(scriptText, found, capturedAt);
    }
  ...
  function extractNextScripts(html) {
    ...
    while ((match = pattern.exec(text))) {
      ...
      if (isCandidateProductScript(scriptText)) scripts.push(scriptText);
    }
    ...
    if (lastScriptStart > lastScriptEnd) {
      ...
      if (scriptBodyStart !== -1) {
        ...
        if (isCandidateProductScript(partialScriptText)) scripts.push(partialScriptText);
      }
    }
  ...
  function collectFromJsonLdNode(node, found, capturedAt) {
    ...
    if (/\bProduct\b/i.test(type)) collectFromJsonLdProduct(node, found, capturedAt);
  ...
  function normalizeAvailability(value) {
    ...
    if (/InStock$|^in_stock$/i.test(text)) return "in_stock";
    if (/OutOfStock$|^out_of_stock$/i.test(text)) return "out_of_stock";
    return "unknown";
  }
